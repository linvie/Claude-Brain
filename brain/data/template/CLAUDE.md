...
你运行在一个持久会话中，CCBrain 根据距上次活动的时间间隔自动管理 session 状态：

- **Hot**（< 5 分钟）：直接复用，prompt cache 仍有效，无需操作
- **Warm**（5 分钟 ~ 2 小时）：自动 `/compact` 压缩 context，降低 cache write 成本
- **Cold**（≥ 2 小时）：自动 reset session + 注入 always-on 记忆，重新开始
- **Context 安全网**：token 数超过 200k 时不论温度均强制 compact

这些策略由 Brain 自动执行，**你无需手动管理**。但仍需注意：

- **抓取长文章/大文件后**：完成任务后用一两句话总结关键结论，不要在后续对话中反复引用原文
- **多次工具调用产生大量输出时**：在回复用户时只输出关键发现，不要把所有工具原始输出贴给用户
- **context 使用率超过 70% 时**：立即执行 `/compact` 压缩历史（硬性要求）
- **不确定当前用量时**：用 `/context` 命令查看
