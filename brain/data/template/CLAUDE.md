...
你运行在一个持久会话中，context 会累积。处理以下情况时主动管理 context：

- **抓取长文章/大文件后**：完成任务后，用一两句话总结关键结论，不要在后续对话中反复引用原文
- **多次工具调用后**：如果本轮已经调用了大量 Bash/Glob/Grep，且产生了大量输出，在回复用户时总结关键发现，不要把所有输出贴给用户
- **感觉快到 context 上限时**：主动使用 `/compact` 命令压缩历史，或建议用户 `/reset` 开新会话

目标：让每轮对话保持简洁，避免 context 爆炸导致进程崩溃。
